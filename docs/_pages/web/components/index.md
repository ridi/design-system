...
리스트는 텍스트와 이미지가 수직으로 연속되어 나열되는 컴포넌트입니다. 전체 페이지를 차지하는 리스트 뿐 아니라 특정 영역을 차지하는 리스트 또한 리스트 컴포넌트에 속합니다.
![List](list/list-intro.png)

### 리스트 구성
![List 구성](list/list-anatomy.png)
크게 리스트열, 리스트 내 콘텐츠, 하단선으로 구성됩니다.

### 리스트 열

- 연속되는 여러 리스트 열은 일관성 있는 포맷을 사용해야 합니다.
- 리스트 열의 순서는 최대한 예측 가능하게 정렬되어야 합니다.
...
- 텍스트만 있는 경우
- 텍스트와 도움을 주는 이미지가 있는 경우

### 하단선

- 리스트 내 콘텐츠가 한 줄일 경우 하단 선을 표시하지 않습니다.
- 모든 최하단 리스트 열의 하단 선은 표시하지 않습니다.

### 리스트 내 타이포그래피

리스트의 주 콘텐츠는 텍스트입니다. 여러 리스트 열을 분류할 때도 `Subtitle`, `Subheader` 같은 종류의 텍스트가 사용됩니다. 그러므로 여러 텍스트 간의 위계가 보다 중요합니다. 텍스트 스타일이 지정된 타입을 용도에 맞게 사용합니다. 
