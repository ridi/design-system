...
- 공간 제약이 있어 레이블을 표시할 수 없는 경우나 버튼을 2개 이상 배치할 경우 사용합니다.
...
많은 양의 콘텐츠가 여러 페이지로 나뉘어 있을 때 페이지네이션으로 다른 페이지로 이동할 수 있습니다.

![페이지네이션 동작](pagination/pagination-ux.png)

### 동작

#### 다음/이전

- **> (다음)** 버튼을 눌렀을 경우 다음 페이지네이션 순서의 가장 처음에 위치한 페이지 번호로 이동합니다.
- **< (이전)** 버튼을 눌렀을 경우 이전 페이지네이션 순서의 가장 처음에 위치한 페이지 번호로 이동합니다.

#### 마지막/처음

- **마지막** 버튼을 눌렀을 경우 마지막 페이지로 이동합니다.
- **처음** 버튼을 눌렀을 경우 처음 페이지로 이동합니다.
- 모바일의 경우 화면 공간 제약상 마지막, 처음 버튼을 표시하지 않습니다.
...
- 페이지가 2개 미만인 경우 페이지네이션은 노출하지 않습니다.
- URL에서 음수나 한글 입력 시 첫 페이지로 이동합니다.
- 페이지 범위 밖의 숫자 입력 시 마지막 페이지로 이동합니다.
