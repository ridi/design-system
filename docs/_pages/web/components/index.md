...
- 1개의 탭
- 2개의 탭
  - 주요 탭은 `primary tab`, 주요 탭에 속한 탭은 `nested tab`이라고 부릅니다.

#### 탭 아이템 가로 길이에 따른 분류

- 전체 길이를 탭 개수로 나눠서 탭 아이템이 다 같은 너비인 경우 `fitted tab` 라고 부릅니다.
- 탭 아이템 내부의 텍스트 레이블의 비율에 맞춰서 다 다른 너비를 사용하는 경우 `clustered fixed tab` 이라고 부릅니다.
  - 언제 이 탭을 써야 하나요?
    - 탭 아이템이 4개 이상이고 다섯 글자 이상인 텍스트 레이블이 1-2개 있을 때
    - 위 케이스 외에도 `fitted tab`으로 구성했는데 불균형해 보인다면 해당 탭을 사용합니다.
...
- 모바일에서 하나의 탭 크기는 개수/전체 길이로 지정합니다.
